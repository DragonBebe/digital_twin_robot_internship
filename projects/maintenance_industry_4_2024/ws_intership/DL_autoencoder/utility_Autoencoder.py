...
class FaultDetectReg():
    ...
    def __init__(self, reg_mdl, pre_trained=True, threshold=1, abnormal_limit=3, window_size=1, sample_step=1, pred_lead_time=1):
    ...
    def predict(self, df_x_test, y_response_test):
        ...
        for name in tqdm(sequence_name_list):
            ...
            y_label_pred_tmp = np.zeros(len(df_x_test_seq) - self.window_size + 1)
            y_temp_pred_tmp = np.zeros(len(df_x_test_seq) - self.window_size + 1)

            for i in range(self.window_size, len(df_x_test_seq) + 1):
                tmp_df_x = df_x_test_seq.iloc[i - self.window_size:i, :]
                tmp_y_temp_measure = y_temp_local.iloc[i - self.window_size:i]
                ...
                y_label_pred_tmp[i - self.window_size] = tmp_y_label_pred
                y_temp_pred_tmp[i - self.window_size] = tmp_y_temp_pred

                if tmp_y_label_pred == 1 and tmp_residual <= self.abnormal_limit:
                    y_temp_local.iloc[i - 1] = tmp_y_temp_pred

            y_label_pred.extend(y_label_pred_tmp.tolist())
            y_response_pred.extend(y_temp_pred_tmp.tolist())

        return y_label_pred, y_response_pred

    def predict_label_by_reg_base(self, X, y_temp_measure):
        y_temp_pred = self.reg_mdl.predict(X)
        residual_error = np.abs(y_temp_pred - y_temp_measure)

        y_label_pred = (residual_error <= self.threshold).astype(int)
        ...
        # Ensure returning scalars by taking the last element
    ...
    def run_cross_val(self, df_x, y_label, y_response, n_fold=5, single_run_result=True):
        ...
        for train_index, test_index in kf.split(test_conditions):
            ...
            _, y_test = prepare_sliding_window(df_x=df_test, y=y_test, window_size=self.window_size,
                                               sample_step=self.sample_step, prediction_lead_time=self.pred_lead_time, mdl_type='clf')
            ...
            if single_run_result:
                if self.pre_trained:
                    ...
                    _, y_response_test = prepare_sliding_window(df_x=df_test, y=y_response_test, window_size=self.window_size,
                                                                sample_step=self.sample_step, prediction_lead_time=self.pred_lead_time, mdl_type='reg')
                    ...
                    _, y_tr = prepare_sliding_window(df_x=df_tr, y=y_tr, window_size=self.window_size,
                                                     sample_step=self.sample_step, prediction_lead_time=self.pred_lead_time, mdl_type='clf')
                    _, y_response_tr = prepare_sliding_window(df_x=df_tr, y=y_response_tr, window_size=self.window_size,
                                                              sample_step=self.sample_step, prediction_lead_time=self.pred_lead_time, mdl_type='reg')
                    _, y_response_test = prepare_sliding_window(df_x=df_test, y=y_response_test, window_size=self.window_size,
                                                                sample_step=self.sample_step, prediction_lead_time=self.pred_lead_time, mdl_type='reg')

                    show_reg_result(y_tr=y_response_tr, y_test=y_response_test, y_pred_tr=y_response_tr_pred,
                                    y_pred=y_response_test_pred, threshold=self.threshold)
...
def run_cv_one_motor(motor_idx, df_data, mdl, feature_list, n_fold=5, threshold=3, window_size=1, sample_step=1,
                     prediction_lead_time=1, single_run_result=True, mdl_type='clf'):
    ''' ### Description
...
def prepare_sliding_window(df_x, y, sequence_name_list=None, window_size=1, sample_step=1, prediction_lead_time=1,
                           mdl_type='clf'):
    ''' ## Description
    ...
    for name in sequence_name_list:
        ...
        for i in range(window_size, len(df_tmp) + 1):
            X_window, y_window = concatenate_features(df_input=df_tmp.iloc[i - window_size:i, :],
                                                      y_input=y_tmp.iloc[i - window_size:i],
                                                      X_window=X_window, y_window=y_window, window_size=window_size,
                                                      sample_step=sample_step,
                                                      prediction_lead_time=prediction_lead_time, mdl_type=mdl_type)
...
def run_cross_val(mdl, df_x, y, n_fold=5, threshold=3, window_size=1, sample_step=1, prediction_lead_time=1,
                  single_run_result=True, mdl_type='reg'):
    ''' ## Description
    ...
    for train_index, test_index in kf.split(test_conditions):
        ...
        X_train, y_train = prepare_sliding_window(df_x, y, names_train, window_size=window_size,
                                                  sample_step=sample_step, prediction_lead_time=prediction_lead_time,
                                                  mdl_type=mdl_type)
        X_test, y_test = prepare_sliding_window(df_x, y, names_test, window_size, sample_step=sample_step,
                                                prediction_lead_time=prediction_lead_time, mdl_type=mdl_type)
...
def show_reg_result_single_run(y_true, y_pred, axes, suffix='training', threshold=3):
    ...
    axes[0].setTitle(f'Regression results on the {suffix} dataset', fontsize=20)
    ...
    axes[1].setTitle(f'Residual errors on the {suffix} dataset', fontsize=20)
    ...
    axes[2].setTitle(f'Distribution of residual errors ({suffix})', fontsize=20)
    ...
    print(f'{suffix} performance, residual error > {threshold}: ' + str(
        sum(abs(y_true - y_pred) > threshold) / y_true.shape[0] * 100) + '%')
...
def show_clf_result(y_tr, y_test, y_pred_tr, y_pred):
    ...
    # ax.set_xlabel('index of data point', fontsize = 15)
    # ax.set_ylabel('y', fontsize = 15)
    # ax.setTitle('Training: Truth', fontsize = 20)
    # ax.plot(range(len(y_tr)), y_tr, 'xb', label='Truth')
    # ax.legend()
    ...
    # ax.set_xlabel('index of data point', fontsize = 15)
    # ax.set_ylabel('y', fontsize = 15)
    # ax.setTitle('Training: Prediction', fontsize = 20)
    # ax.plot(range(len(y_pred_tr)), y_pred_tr, 'or', label='Prediction')
    # ax.legend()
    ...
    # ax.set_xlabel('index of data points', fontsize = 15)
    # ax.set_ylabel('y', fontsize = 15)
    # ax.setTitle('Testing: Truth', fontsize = 20)
    # ax.plot(range(len(y_test)), y_test, 'xb', label='Truth')
    # ax.legend()
    ...
    # ax.set_xlabel('index of data points', fontsize = 15)
    # ax.set_ylabel('y', fontsize = 15)
    # ax.setTitle('Testing: Prediction', fontsize = 20)
    # ax.plot(range(len(y_pred)), y_pred, 'or', label='Prediction')
    # ax.legend()
...
def show_clf_result_single_run(y_true, y_pred, ax_tr, ax_pred, suffix='training'):
    ...
    ax_tr.setTitle(f'{suffix}: Truth', fontsize=20)
    ...
    ax_pred.setTitle(f'{suffix}: Prediction', fontsize=20)
...
if __name__ == '__main__':
    ...
    x_tr_org, y_temp_tr_org = extract_selected_feature(df_data=df_tr, feature_list=feature_list_all, motor_idx=6,
                                                       mdl_type='reg')
    ...
    x_tr, y_temp_tr = prepare_sliding_window(df_x=x_tr_org, y=y_temp_tr_org, window_size=window_size,
                                             sample_step=sample_step, prediction_lead_time=prediction_lead_time,
                                             mdl_type='reg')
    ...
    _, y_label_test_org = extract_selected_feature(df_data=df_test, feature_list=feature_list_all, motor_idx=6,
                                                   mdl_type='clf')
    x_test_org, y_temp_test_org = extract_selected_feature(df_data=df_test, feature_list=feature_list_all, motor_idx=6,
                                                           mdl_type='reg')
    ...
    _, y_label_test = prepare_sliding_window(df_x=x_test_org, y=y_label_test_org, sequence_name_list=test_id,
                                             ...
                                             prediction_lead_time=prediction_lead_time, mdl_type='clf')
    _, y_temp_test_seq = prepare_sliding_window(df_x=x_test_org, y=y_temp_test_org, sequence_name_list=test_id,
                                                window_size=window_size, sample_step=sample_step,
    ...
    show_reg_result(y_tr=y_temp_test_seq, y_test=y_temp_test_seq, y_pred_tr=y_temp_pred_tmp, y_pred=y_temp_pred_tmp,
                    threshold=detector_reg.threshold)
