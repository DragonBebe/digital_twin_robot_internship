...
def run_cv_one_motor_OCSVM(motor_idx, df_data, mdl, feature_list, n_fold=5, threshold=3, window_size=1, sample_step=1,
    ...
    print(f'Model for motor {motor_idx}:')
    # Run cross-validation.
    df_perf = run_cross_val_OCSVM(mdl, df_x, y, n_fold=n_fold, threshold=threshold, window_size=window_size,
    ...
    plt.errorbar(df_perf.columns, df_perf.mean(), yerr=df_perf.std(), fmt='o', capsize=5)
    plt.xlabel('Metrics')
    plt.ylabel('Scores')
    plt.title(f'Performance metrics for motor {motor_idx}')
    plt.show()

    return df_perf


def run_cross_val_OCSVM(mdl, df_x, y, n_fold=5, threshold=3, window_size=1, sample_step=1, prediction_lead_time=1,
...
class FaultDetectIsoForest:
    def __init__(self, iso_forest, threshold=1, window_size=1, sample_step=1, pred_lead_time=1):
    ...
    def fit(self, df_x, y_label):
        df_x_normal = df_x[y_label == 0]
        x_tr, _ = prepare_sliding_window(df_x=df_x_normal, y=y_label[y_label == 0], window_size=self.window_size, sample_step=self.sample_step, prediction_lead_time=self.pred_lead_time, mdl_type='clf')
        self.iso_forest.fit(x_tr)

    def predict(self, df_x_test, y_label_test):
        window_size = self.window_size
        sample_step = self.sample_step
        sequence_name_list = df_x_test['test_condition'].unique().tolist()
        y_label_pred = []
        ...
        for name in sequence_name_list:
            df_x_test_seq = df_x_test[df_x_test['test_condition'] == name]
            y_label_pred_tmp = np.zeros(len(df_x_test_seq) - window_size + 1)

            for i in range(window_size, len(df_x_test_seq) + 1):
                tmp_df_x = df_x_test_seq.iloc[i - window_size:i, :]
                feature_x, _ = concatenate_features(df_input=tmp_df_x, y_input=y_label_test.iloc[i - window_size:i], X_window=[], y_window=[], window_size=window_size, sample_step=sample_step, prediction_lead_time=self.pred_lead_time, mdl_type='clf')
                tmp_y_label_pred = self.iso_forest.predict(feature_x)
                tmp_y_label_pred = np.where(tmp_y_label_pred == 1, 0, 1)
                y_label_pred_tmp[i - window_size] = tmp_y_label_pred[-1]
...
def run_cross_val_iso_forest(detector, df_x, y, n_fold=5):
    test_conditions = df_x['test_condition'].unique().tolist()
    kf = KFold(n_splits=n_fold)
    perf = np.zeros((n_fold, 4))
    ...
    for train_index, test_index in kf.split(test_conditions):
        ...
        detector.fit(df_tr, y_train)
        y_pred = detector.predict(df_test, y_test)

        # Align lengths of y_test and y_pred
        y_test_aligned = y_test[window_size - 1:].iloc[:len(y_pred)]  # Adjust the length to match y_pred
...
def run_cv_one_motor_iso_forest(motor_idx, df_data, iso_forest, feature_list, n_fold=5, threshold=1, window_size=1, sample_step=1, prediction_lead_time=1, single_run_result=True):
    df_x, y = extract_selected_feature(df_data, feature_list, motor_idx, 'clf')
    print(f'Model for motor {motor_idx}:')

    detector = FaultDetectIsoForest(iso_forest=iso_forest, threshold=threshold, window_size=window_size, sample_step=sample_step, pred_lead_time=prediction_lead_time)
    df_perf = run_cross_val_iso_forest(detector, df_x, y, n_fold=n_fold)
